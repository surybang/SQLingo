...
def create_cross_joins_exercises(con: duckdb.DuckDBPyConnection) -> None:
    beverages_df = pd.read_csv(  # noqa: F841
        io.StringIO("beverage,price\norange juice,2.5\nExpresso,2\nTea,3\n")
    )
    con.execute("CREATE TABLE IF NOT EXISTS beverages AS SELECT * FROM beverages_df")

    food_items_df = pd.read_csv(  # noqa: F841
        io.StringIO("food_item,food_price\ncookie,2.5\ncholatine,2\nmuffin,3\n")
    )
    ...
    trademarks_df = pd.read_csv(  # noqa: F841
        io.StringIO("trademark\nNike\nAsphalte\nAbercrombie\nLewis\n")
    )
...
def create_inner_joins_exercises(con: duckdb.DuckDBPyConnection) -> None:
    salaries_df = pd.read_csv(  # noqa: F841
        io.StringIO("salary,employee_id\n2000,1\n2500,2\n2200,3\n")
    )
...
def create_left_joins_exercises(con: duckdb.DuckDBPyConnection) -> None:
    ...
    products_df = pd.DataFrame(  # noqa: F841
        {
    ...
    order_details_df = pd.DataFrame(  # noqa: F841
        {
            "order_id": [1, 2, 3, 4, 5],
    ...
    con.execute(
        "CREATE TABLE IF NOT EXISTS order_details AS SELECT * FROM order_details_df"
    )
...
def create_full_outer_joins_exercises(con: duckdb.DuckDBPyConnection) -> None:
    ...
    stores_df = pd.DataFrame(  # noqa: F841
        {"store_id": [1, 2, 3, 4], "customer_id": [11, 12, 13, 15]}
    )
    con.execute("CREATE TABLE IF NOT EXISTS df_stores AS SELECT * FROM stores_df")

    store_products_df = pd.DataFrame(  # noqa: F841
        {
...
def create_self_joins_exercises(con: duckdb.DuckDBPyConnection) -> None:
    employees_df = pd.DataFrame(  # noqa: F841
        {
...
def create_group_by_exercises(con: duckdb.DuckDBPyConnection) -> None:
    ventes_immo_df = pd.DataFrame(  # noqa: F841
        [
...
def create_case_when_exercises(con: duckdb.DuckDBPyConnection) -> None:
    salaires_df = pd.DataFrame(  # noqa: F841
        {
            "name": [
                "Toufik",
                "Jean-Nicolas",
    ...
    discount_df = pd.DataFrame(  # noqa: F841
        {
...
def create_grouping_set_exercises(con: duckdb.DuckDBPyConnection) -> None:
    redbull_df = pd.DataFrame(  # noqa: F841
        {
    ...
    datapop_df = pd.DataFrame(  # noqa: F841
        {
